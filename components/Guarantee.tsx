...
const icons = [ShieldCheck, CalendarX, Coins];

export default function Guarantee() {
  const { lang } = useLang();
  const t = tx(lang).guarantee;
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-8">
        ...
        <div className="mb-10 md:mb-16 text-center">
          <div className="flex items-center justify-center gap-2.5 mb-5">
            ...
            <p className="text-[11px] font-normal uppercase tracking-[0.25em] text-[#8e8e93]">{t.tag}</p>
          </div>
            ...
            return (
              <div key={card.title} className="bg-white rounded-3xl p-7 flex flex-col gap-5 border border-[#e8e8e8]">
                <div className="w-11 h-11 rounded-2xl bg-[#E8440A]/10 flex items-center justify-center">
                  <Icon className="w-5 h-5 text-[#E8440A]" strokeWidth={1.5} />
                </div>
        ...
        <div className="mt-10 text-center">
          ...
          <p className="text-xs text-[#a0a0a8] font-normal mt-2">{t.quoteAuthor}</p>
        </div>
