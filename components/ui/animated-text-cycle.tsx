...
export default function AnimatedTextCycle({
  ...
  const containerVariants = {
    ...
    visible: { y: 0,   opacity: 1, filter: "blur(0px)", transition: { duration: 0.4, ease: [0.4, 0, 0.2, 1] as const } },
    exit:    { y: 20,  opacity: 0, filter: "blur(8px)", transition: { duration: 0.3, ease: [0.4, 0, 1, 1] as const } },
  };
  ...
  return (
    <>
      <div ref={measureRef} aria-hidden="true" className="absolute opacity-0 pointer-events-none" style={{ visibility: "hidden" }}>
        {words.map((word, i) => (
          <span key={i} className={`font-semibold ${className}`}>{word}</span>
        ))}
      </div>
      <motion.span
