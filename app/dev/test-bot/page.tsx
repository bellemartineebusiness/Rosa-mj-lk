  ...
  async function send() {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        role: "assistant",
        content: res.ok ? (d.reply ?? "Något gick fel.") : (d.error ?? "Något gick fel."),
      }]);
    } catch {
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f7] flex items-center justify-center px-6">
      <div className="flex flex-col items-center gap-4">
        <p className="text-xs text-[#8e8e93] uppercase tracking-widest">Testläge — Salon Aurora</p>
        ...
        <div className="w-80 bg-white rounded-3xl shadow-2xl shadow-black/15 border border-[#e8e8e8] overflow-hidden flex flex-col">
          ...
          <div className="h-96 overflow-y-auto px-4 py-4 flex flex-col gap-2.5 bg-[#f9f9f9]">
            {messages.map((msg, i) => (
